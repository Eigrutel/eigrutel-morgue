...
def prepare_identity():
    ...
    if function('EigrutelLab.Morgue.Desktop') < 0:
        raise OSError('Windows rejected the Morgue application identity')
...
class WindowIcons:
    ...
    def apply(self, tk_window_id):
        ...
        if not hwnd:
            raise OSError('Native Morgue window not available')
        if not self.handles:
            try:
                for metric_x, metric_y in ((49,50),(11,12)):
                    ...
                    if not handle:
                        raise OSError('Windows could not load Morgue.ico')
        ...
        for kind, handle in enumerate(self.handles):
            ...
            if self.api.SendMessageW(hwnd, 0x7F, kind, 0) != handle:  # WM_GETICON
                raise OSError('Windows did not retain the Morgue icon')
...
def install(root):
    if sys.platform != 'win32':
        return
    from ui_common import app_dir, resource_path
    path = os.path.join(getattr(sys, '_MEIPASS', app_dir()), 'Morgue.ico')
    if not os.path.isfile(path):
        path = resource_path('Morgue.ico')
    try:
        icons = WindowIcons(windows_api(), path)
    except (AttributeError, OSError) as error:
        log_error(error)
        return
    root._native_morgue_icons = icons
    queued = False
    closed = False
    def apply():
        nonlocal queued
