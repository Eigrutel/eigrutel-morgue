...
class IconTests(unittest.TestCase):
    ...
    def test_resource_group_must_contain_the_supplied_images(self):
        payloads=build.read_ico_payloads(BASE/'Morgue.ico')
        directory=(BASE/'Morgue.ico').read_bytes()
    ...
    def test_windows_sets_current_window_and_default(self):
        window=Mock()
        with patch.object(ui_common.sys,'platform','win32'):
            self.assertTrue(ui_common.apply_app_icon(window))
        self.assertEqual(window.iconbitmap.call_count,2)
        self.assertTrue(window.iconbitmap.call_args_list[0].args[0].endswith('Morgue.ico'))
    ...
    def test_failed_ico_uses_png_fallback(self):
        ...
        with patch.object(ui_common.sys,'platform','win32'), patch('PIL.ImageTk.PhotoImage',return_value=object()):
            self.assertTrue(ui_common.apply_app_icon(window))
        self.assertEqual(len(window._app_icons),4)
...
class NativeTaskbarTests(unittest.TestCase):
    ...
    def test_small_and_large_icons_target_native_wrapper(self):
        from morgue_windows_icon import WindowIcons
        api = self.api()
        icons = WindowIcons(api,'Morgue.ico')
        icons.apply(123)
